...
double s21::Calculations::CalculateReversePolish(lexeme *reverse_polish) {
    ...
    while (reverse_polish[index_rev_pol].type != UNDEFINED) {
        ...
        if (reverse_polish[index_rev_pol].type == FUNCTION) {
            numbers[index_num - 1] = CalculateFunction(numbers[index_num - 1], reverse_polish[index_rev_pol].lexeme_kind);
        }

        if (reverse_polish[index_rev_pol].type == OPERATOR) {

            if (reverse_polish[index_rev_pol].unary == 1) {
                numbers[index_num - 1] = CalculateUnaryOp(numbers[index_num - 1], reverse_polish[index_rev_pol].lexeme_kind);
            } else {
                numbers[index_num - 2] = CalculateOp(numbers[index_num - 2], numbers[index_num - 1], reverse_polish[index_rev_pol].lexeme_kind);
...
double s21::Calculations::CalculateOp(double number_1, double number_2, char op) {
    double result = 0;
    switch (op) {
        case '+':
