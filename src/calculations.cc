...
int main() {
    ...
    std::string str = "4 + 5 - (3 + 5)";
    s21::lexeme reverse_polish[LEN];
    calc.ToReversePolish(str, reverse_polish);
    for (int i = 0; i < 1; i++) {
        printf("type=%d\nkind=%d\nnumber=%f\n\n", reverse_polish[i].type, reverse_polish[i].lexeme_kind, reverse_polish[i].number);
    }
    // s21::lexeme lex;
    // lex.type = s21::FUNCTION;
    // lex.lexeme_kind = 's';
    // int n = calc.GetPriority(lex);
    // printf("%d\n", n);
    return 0;
}

void s21::Calculations::ToReversePolish(std::string &str_input, lexeme *reverse_polish) {
    ...
    while (index_input < str_input.length()) {
        ...
        if (lex.type == OPERATOR) {
            lexeme &lexe = queue.top();

            printf("type=%d\nkind=%d\nnumber=%f\n\n", lexe.type, lexe.lexeme_kind, lexe.number);
            // while (queue.top().type == FUNCTION || (queue.top().type == OPERATOR && GetPriority(queue.top()) <= GetPriority(lex))) {
            //     // reverse_polish[index_output] = queue.top();
            //     index_output++;
            //     // queue.pop();
            // }
...
int s21::Calculations::DefineLexeme(std::string &str_input, lexeme *lex, int index_input) {
    ClearLexeme(lex);
    char lex_kind = 0;
    double number = 0;
...
char s21::Calculations::isNum(std::string &str_input, int *index_input, double *number) {
    char status = 0;
    int count_symbol = 0;
...
char s21::Calculations::isFunction(std::string &str_input, int *index_input) {
    ...
    if (new_str == "sin" || new_str == "cos" || new_str == "tan" || new_str == "log") {
        status = str_input[*index_input];
        *index_input += 3;
    }
    else if (new_str == "asin" || new_str == "acos" || new_str == "atan" || new_str == "sqrt") {
